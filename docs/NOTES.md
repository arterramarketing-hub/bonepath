# BONEPATH — the long notes

This is the history behind the rules in `CLAUDE.md`: what broke, how it was
found, and the measurements that settled it. `CLAUDE.md` keeps the rules and
the traps, short, because it is loaded into every session. This file keeps
the reasons, the numbers and the dead ends — read the entry for a system
before you change it, and search here before you re-measure something.

Entries are in the order they were written, not by topic. Search by the
code's own names (`bakeStatic`, `THE ARM`, `xfStart`, `CHAIN_OUT`, ...);
they match the contents list at the top of the game code in `index.html`.

When you add a rule to `CLAUDE.md`, put its story here.

## Branch layout (history)
